...
let resolve_source
  ...
  | None ->
      Error (sprintf "missing ml file for module %s" module_.name)
  | Some impl_path ->
      Ok (relativize (workspace.build_context ^ "/") impl_path)
...
let process_one_cmt
  ...
  | { cmt_sourcefile = None; _ } | { cmt_source_digest = None; _ } -> Ok ()
  | exception Cmt_format.Error (Cmt_format.Not_a_typedtree _) ->
      warning (sprintf "error reading cmt file: %s" cmt_path);
      Error ()
  | exception Sys_error msg ->
      warning (sprintf "system error occurred while reading cmt file: %s: %s"
                 cmt_path msg);
      Error ()
...
let incremental_search
  ...
  if successes < total then (
    ...
    handle_event (
      Warning (sprintf "%d/%d cmt files found (%.1f%% coverage); \
