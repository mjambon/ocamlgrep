...
let strings_field name pairs =
  ...
  | Csexp.List xs ->
    List.map
      ...
      xs
  | _ -> Printf.ksprintf failwith "dune describe: field %S: expected a list" name
...
let interpret_entries entries =
  ...
  | _, None -> Error "dune describe workspace: missing [(build_context ...)] entry"
...
let describe ?context ?dirs ?root () =
  ...
  | exception Unix.Unix_error (err, _, _) ->
    Error (Printf.sprintf "could not run `dune describe workspace`: %s"
  ...
  | ic, oc, ec ->
    ...
    (match Unix.close_process_full (ic, oc, ec) with
     ...
     | WEXITED n ->
       Error (Printf.sprintf "dune describe workspace exited with code %d:\n%s"
                n (String.trim stderr_data))
     | WSIGNALED n ->
